...
--------------------------------------
SELECT INDEX_NAME, TABLE_NAME 	=> '인덱스이름'과 '칼럼이름' 두개의 칼럼을 찾아야함
 FROM USER_INDEXES		* 인덱스 관련 정보는 USER_INDEXES, USER_IND_COLUMNS를 사용
WHERE TABLE_NAME = 'EMP';	=> 1) DESC USER_INDEXES 		=> 인덱스 이름을 확인할 수 있는 칼럼 제공
				=> 2) DESC USER_IND_COLUMNS 	=> 인덱스의 이름과 해당 인덱스가 설정된 칼럼이름도 확인가능
--------------------------------------

				* DESC는 테이블의 구조 / SELECT는 레코드 값 조회에 사용
...
CREATE TABLE NEW_PRODUCT
	AS (SELECT * 
	       FROM PRODUCT
	      WHERE 1 = 2);	=> WHERE 1 = 2 : 레코드값은 안나오고, 칼럼값만 나오게 하는 방법	
------------------------------------			
...
INDEX CREATE AS 테이블(칼럼)
VIEW CREATE AS(SELECT 
...
-----------------------------
SELECT * FROM TAB;		=> 테이블 이름을 조회할 수 있는 칼럼 : TNAME
-----------------------------		
...
--------------------------------------------
SELECT * FROM USER_CONSTRAINTS;		=> SELECT는 테이블의 칼럼, 레코드 확인, DESC는 테이블의 구조 확인
--------------------------------------------

& 정답
--------------------------------------------
=> DESC USER_CONSTRAINTS;		=> 구조만 확인이니 DESC
--------------------------------------------	
...
SELECT DEPT_NAME
	...
	WHERE DEPT_ID = (SELECT DEPT_ID
			 FROM EMPLOYEE				=> SELECT는 AS 별칭, FROM은 그냥 별칭사용, WHERE은 별칭사용 X
...
SELECT EMP_ID, EMP_NAME, SALARY
	...
	WHERE DEPT_ID = (SELECT DEPT_ID
			  FROM DEPARTMENT
			WHERE DEPT_NAME = '총무부');		=> 괄호 하나가 많았음	
----------------------------------------------------------------------
...
* 테이블 생성시 제약사항 작성
(1) 칼럼 옆에 붙이기 		: NOT NULL PRIMARY KEY
(2) 칼럼 아래에 붙이기 		: PRIMARY KEY(칼럼이름)
(3) 칼럼 옆에 이름지어 붙이기	: CONSTRAINT 제약명 PRIMARY KEY
(4) 칼럼 아래에 이름지어 붙이기	: CONSTRAINT 제약명 PRIMARY KEY(칼럼명)
(5) ALTER  사용			: ALTER TABLE 테이블명 ADD CONSTRAINT 제약명 PRIMARY KEY(칼럼명)
