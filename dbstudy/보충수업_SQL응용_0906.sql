...
--PROFESSOR(P)- - DEPARTMENT       --    STUDENT
--PROF(P)          PROF(F),  
                 --DEPT(P)          --DEPT(F)
                                    --STUDNO(P)
                                    
/**
    DEPARTMENT의 값을 학과번호 DEPT를 삭제해야하는데 DEPTNO를 STUDENT가 참조중(삭제불가)
    1. 일단 STUDENT의 DEPTNO가 101인 정보 지우기
    2. DEPTNO에서 DEPTNO가 101인 PROF NO 찾기
    3. DEPARTMENT 테이블에서 DEPT 101인 정보지우기
    4. PROFESSOR에서 2단계에서 찾은 PROFNO 정보 지우기
    


*/

-- 생성순서 : PROFESSOR 만들고, DEPARTMENT 만들고, STUDENT만들기
-- 삭제순서 : STUDENT DEPARTMENT PROFESSOR

-- DEPTNO에 연관된 모든 테이블의 데이터 지우기
-- 1. STUDENT 테이블에서 DEPTNO = 101인 정보 지우기
-- 2. DEPARTMENT에서 DEPTNO= 101인 PROFNO 저장하기                         101인 PROF의 번호 찾기
-- 3. DEPARTMENT 테이블에서 DEPTNO =101 인 정보 지우기                      찾아서 없애기
