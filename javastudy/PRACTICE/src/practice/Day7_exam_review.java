...
//		}
		...
		int balance = 5000;
//		for(int cnt = 1; cnt <= 10; cnt++) {   // * 정해진 숫자 반복이니 for
//			
//		} 
		for(int cnt = 0; cnt < 10; cnt++) {   // => for문에서 중요한건 횟수 * 더 자주씀
			int money = (int)(Math.random() * 1000) +1;
			System.out.println("출급 전" + balance + "원, " + (cnt + 1) +"회 출금" + money + "원, 출금 후" + (balance -= money) + "원"); // *
			// * balance -= money는 실행문에서 직접 표현하면 된다 
//			if(balance < 0) {
//				break;
...
//			} System.out.println(str3 + "에 포함된 h는" + count + "개입니다");
		...
		String str3 = sc3.next();  // * 굳이 공백넣을필요x
		for(int i = 0; i < str3.length(); i++) {
			if(str3.charAt(i) == 'h') {
				cnt++;
			}
		} System.out.println(str3 + "에 포함된 h는" + cnt + "개입니다");
...
//		System.out.println("변환 후 파일명 = " + afterName);
		...
		String beforeName2 = sc5.next();				// * 문제에서 next로 하라했다
		String[] arr = beforeName2.split("\\.");	// {"happy" , "jpg"}
		// string 클래스 split 메소드 : 표현식 (ex마침표)를 기준으로 쪼개기
		// 자바에서는 역슬래시를 두개 표현해야함									// arr  {aaa, bbb, jpg}
		String[] temp = new String[arr.length -1]; // 								// temp {"", ""}
		System.arraycopy(arr, 0, temp, 0, temp.length);							// temp {aaa, bbb}
		
		String extName2 = arr[arr.length - 1];
		//extName2 = "_" + System.currentTimeMillis() + "." + extName2; // extName2 = _timestamp.jpg
		String fileName = String.join(".", temp);  						// fileName = "aaa, bbb"
		// * System.currentTimtMillis => 타임 스탬프 *
		fileName += "_" + System.currentTimeMillis() + fileName; //fileName = aaa bbb _timestamp
		String afterName2 = fileName + "." +extName2;
		System.out.println("변환 후 파일명 = " + afterName2);
		
		// split와 join을 이용해 특정기호 제거하기
		//String tel = "010-1234-5678";
		//String[] arr5 = tel.split(("-");	// {"010, "1234", "5678}
		//tel = String.join("", arr5);		// {01012345678}		
...
//		String name3 = "한지민";
		...
		Scanner sc2 = new Scanner(System.in);
		System.out.println("이름 입력 >>>  ");		//* 안내메시지 위치 주의
		String name2 = sc2.next();
		if(name2.equals("전지현") || name2.equals("정우성") || name2.equals("한지민")) {
			System.out.println("반갑다 친구야");
		} else {
			System.out.println("처음 뵙겠습니다");
		}
...
//			if(name3.equals(friend) {
//				System.out.println("반갑다 친구야");
//				//break;
//				return;
//				// => 메소드 종료 
//				// q9 자체가 종료되서 밑에 실행문이 이행되지 않는다
//			} 
//		} System.out.println("처음 뵙겠습니다");		// * 위치가 애매함
//		
		// --------------------------------------------------------------------
		// switch문 활용 *
		
		switch(name) {
		case "전지현" : case "정우성" : case "한지민" : System.out.println("반갑다 친구야"); break;
		default : System.out.println("처음 뵙겠습니다");
		}
	...
	public static void q10() {
		...
		for(int cnt = 0; cnt <5; cnt++) {
			System.out.println("비밀번호 입력(cnt + 1) >>>");		// * 5번 반복할 실행문 입력
			String pw2 = sc5.next();
			if(pw2.equals("1234abcd")) {
				System.out.println("성공");
				break; // return 도 좋음
			} else {
				if(cnt == 4) {										// => 4에서 성공못하고 내려오면 5번째 실패
					System.out.println("횟수초과");
				} else {
					System.out.println("실패");
				}
