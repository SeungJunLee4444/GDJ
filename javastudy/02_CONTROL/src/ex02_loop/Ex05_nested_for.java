...
public class Ex05_nested_for {
	...
	public static void main(String[] args) {
		...
		// 연습
		// 전체 구구단 a x b = 0
		// 해석 => 곱해야할 두개의 값과 최종값이 있어야 하므로 세가지의 변수가 선언되야 한다 *
		// 사전에 선언해야할 변수에 대해 고려하는 습관으로 연습문제를 풀것 **
		int a;
		int b;
		int sum = 0;
		for(a = 1; a <= 9; a++) {
			for(b = 1; b <=9; b++) {
				sum = a * b;
				System.out.println(a + "x" + b + "=" + sum );	// => 애는 꼭 안에 들어가야한다
			}
		}
		...
		label : for(c = 2; c <=9; c++) { // => c가 6이되도 조건식이 5보다 작거나 같음이면 멈춤
			for(d = 1; d <=9; d++) {
				...
				} // if(c == 5 && d == 5) => 바깥에 한번더 붙이는 법도 가능하다
		...
		for(e = 2; e <= 9; e++) {
			for(f = 1; f <=9; f++) {
				System.out.println(f + "x" + e + "=" + (e * f));	// => 애는 꼭 안에 들어가야한다
			} 
		}
		...
		for (da = 1; da <= 9; da++) { // => 바깥함수가 고정되는 형태 
			for(dan = 2; dan <=9; dan++) {
				...
				if(dan == 9) {  // => 변수명을 이해하기 쉽게 지으면 뒤바뀐 상황에서도 빨리 캐치 *
